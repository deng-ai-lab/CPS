...
class EnKG(Algo):
        ...
        for i, (t_cur, t_next) in tqdm(
            ...
            if i < (self.num_steps - int(0.5 * self.threshold)) and i > self.threshold:
                x_hat, t_hat = self.update_particles(
                    x_cur,
                    observation,
                    num_steps=min(1 + (self.num_steps - i) // self.factor, 1),
                    sigma_start=t_cur,
            ...
            print(f"{i}/{len(t_steps)}")
            # Euler step
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur
        return x_next[0][None,...]
    ...
    def update_particles(self, particles, observation, num_steps, sigma_start, guidance_scale=1.0):
        ...
        for j in range(self.num_updates):
            ...
            ####
            # denoised_mean = self.net((x_cur * scaling_factor + factor * score) / self.scheduler.scaling_steps[i+1], torch.as_tensor(sigma_next).to(x_cur.device))
            denoised_mean = ode_sampler(
                    self.net,
                    particles.mean(dim=0,keepdim=True),
                    num_steps=num_steps,
                    sigma_start=sigma_start,
                )
            
            y_mean = self.forward_op.forward(denoised_mean)
            MSE= torch.mean(ys.mean(dim=0, keepdim=True)-y_mean)
            print("MSE:",MSE.item())
            ####
